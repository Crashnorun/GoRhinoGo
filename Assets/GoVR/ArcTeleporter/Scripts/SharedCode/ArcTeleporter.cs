...
public class ArcTeleporter : MonoBehaviour {
	...
	void Update () {
		...
		bool currentTriggerState = transform.parent.GetComponent<PlayerControl>().lastTouchPress;

		// If the touchpad press was released this frame, but only if trigger is not pressed
		if (lastTriggerState && !currentTriggerState && !transform.parent.GetComponent<PlayerControl>().lastTriggerState) {
			...
			if (arcRaycaster!= null && arcRaycaster.MakingContact) {
				if (objectToMove != null) {
                    ...
                    //make sure client knows that it has teleported
                    transform.parent.GetComponent<PlayerControl>().teleported = true;
                }
			}
